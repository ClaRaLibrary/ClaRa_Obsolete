...
public
  ...
  parameter Real CL_Delta_pIP_mLS[:,:]=[0,0;0.1, 0.01; 0.2, 0.04; 0.3, 0.09; 0.4, 0.16; 0.5, 0.25; 0.6, 0.36; 0.7, 0.49; 0.8, 0.64; 0.9, 0.81; 1, 1] "Characteristic line of reheat pressure drop as function of mass flow rate"
                                                                                annotation(Dialog(group="Part Load Definition"));
  parameter Real CL_yF_QF[:,:] = [0,0.9;1,0.9] "Characteristic line of relative heat release in life steam as function of rel. firing power"
                                                                                    annotation(Dialog(group="Part Load Definition"));
  parameter Real CL_etaF_QF[:,:] = [0,0.93;1,0.94] "Characteristic line of furnace efficiency as function of rel. firing power"
                                                                                 annotation(Dialog(group="Part Load Definition"));
...
public
  ...
  ClaRa.Basics.Interfaces.EyeOut eye_LS if showData annotation (Placement(transformation(
        extent={{-6,-6},{6,6}},
        rotation=90,
        origin={-40,226}), iconTransformation(
        extent={{-6,-6},{6,6}},
        rotation=90,
        origin={-40,226})));
protected
  ClaRa.Basics.Interfaces.EyeIn eye_intLS annotation (Placement(transformation(extent={{-43,159},{-41,161}})));
public
  ClaRa.Basics.Interfaces.EyeOut eye_RH if showData annotation (Placement(transformation(
        extent={{-6,-6},{6,6}},
        rotation=90,
...
// balance equations - definition of live steam and reheated steam conditions
  ...
  h_evap=h_inHP+Q_flow_evap/feedwater.m_flow;
  Q_flow_evap=Q_flow_HP*convert2Evap.y[1];

   //der(h_evap)= (feedwater.m_flow*h_inHP + m_flow_heatedHP*h_evap + Q_flow_evap + der(p_HP)*volume_tot_HP - drhodt_HP*volume_tot_HP*h_HP)/mass_HP;
...
equation
  ...
  connect(eye_RH,eye_intRH)
      ...
      thickness=0.5,
      smooth=Smooth.None));
  connect(heatRelease.y, convert2Evap.u[1])      annotation (Line(
      points={{-45,-46},{-30,-46},{-30,16},{-12,16}},
      color={0,0,127},
      smooth=Smooth.None));
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false,extent={{-100,
          ...
          extent={{-100,-100},{100,220}}), graphics={
                                          Polygon(
          points={{-100,-20},{100,180},{-100,-20}},
          lineColor={255,0,0},
          smooth=Smooth.None,
          fillColor={102,198,0},
          fillPattern=FillPattern.Solid),Polygon(
          points={{-100,180},{100,-20},{-100,180}},
          lineColor={255,0,0},
          smooth=Smooth.None,
          fillColor={102,198,0},
