...
//___________________________________________________________________________//
  ...
  parameter Modelica.SIunits.MassFlowRate m_flow_nom_spray=10 "Nominal injection mass flow rate"
                                        annotation(Dialog(group="Nominal Values"));
  parameter Modelica.SIunits.Pressure Delta_p_nozzle=1000 "Nominal pressure loss of spray valve"  annotation(Dialog(group="Nominal Values"));
...
public
  ClaRa.Components.VolumesValvesFittings.Fittings.Join_L2_flex mixingZone(
    N_ports_in=2,
    medium=medium,
    ...
    preciseTwoPhase=preciseTwoPhase) annotation (Placement(transformation(extent={{-18,10},{2,30}})));
